...
public sealed class RoadAreaDrawingSession
{
	...
	public AddMapObjectCommand Finish(MapDocument map, Guid? activeTargetLayerId = null)
	{
		if (!CanFinish)
		{
			throw new InvalidOperationException("Road area requires at least three points.");
		}
		AddMapObjectCommand result = MapObjectCreationService.CreateRoadAreaCommand(map, _points, activeTargetLayerId);
		Cancel();
		return result;
	}
